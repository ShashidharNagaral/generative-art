...
const placeObjectRandomlyWithoutOverlapping = (p, w, h, size, objects) => {
  ...
  do {
    ...
    attempt++;
  } while (!isValidPosition(p, pos[0], pos[1], size, objects) && attempt < 100);
  if (attempt >= 100) {
...
/*
1. check for first object with all the other objects
2. if we find no collision then go for second object and dont check when the first one
*/
...
const MultipleCollision = () => {
  useEffect(() => {
    const p = new p5(sketch, document.getElementById("multiple-collision"));
    return () => {
      p.remove();
    };
  }, []);
  return <div id="multiple-collision"></div>;
};

export { MultipleCollision };
